###Instructions -- email from Rolf Lohaus
#To use the script source it and call the function 'analyzeGrowthCurves(dataFileName="xyz.txt")'.  It generates an overview figure (xyz.pdf) and a results text file (xyz_analysis.txt).  The growth rates in the results file should be per hour and the lag times are in seconds (if no second growth phase was detected for a well, the respective columns contain "NA" values).

#Adding the parameter 'plotDetail=TRUE' to the call of 'analyzeGrowthCurves()' produces a PDF 'xyz_detail.pdf' with additional information and pages showing the response in growth rate and other measures used in the analysis.

#I also added the functionality to plot the results for just one well in higher resolution (e.g. to double check the fit of the estimates in more detail for a specific well).  Use the additional parameter 'singleWell' (e.g. 'singleWell="H4"') and the function generates  e.g. 'xyz_single_H4.pdf'.

#The script for now estimates two slightly different 2nd lag times.  The first one (LagTime2) measures it from the inflection point (or the valley in growth rate, red circle in plots) between the two points of maximum growth rate (green circles in plots), and the second one (LagTime2Ext) measures it from the point of maximum OD to the left of the inflection point if there is one (thus the second measure might estimate a slightly longer 2nd lag time than the first measure).  It's kinda hard to explain, you can see it from the green horizontal bar in the example figure mentioned above.  I wasn't really sure at which y/OD value to measure the 2nd lag time, for now it's measured at the OD value of the inflection point (for both measures), let me know if you want that changed.
...
#1. Highlight everything in this file and copy to terminal -- then press return

#Copy this line into 'Terminal', editing the first term to point to the relevant input file, and press return.
#2. analyzeGrowthCurves("20111221_erythromycin_vs_nalidixic.txt", twoResourceWells=c("NONE"), maxTimeHours=24, timeTicks=c(0,12,24), maxOD=0.2, ODTicks=c(0.1,0.2), smoothWindowSize=20)
#######################

#Here is a quick explanation of the additional parameters:

#- twoResourceWells = c("NONE"):  by default the script expects diauxic growth in all wells and tries to identify the two growth phases.  

#- maxTimeHours = 24:  sets the maximum x-axis value to 61 hours

#- timeTicks = c(0,12,24):  sets the x-axis tick marks to the given values

#- maxOD = 0.2:  sets the maximum y-axis value to 0.72 OD

#- ODTicks = c(0.1,0.2):  sets the y-axis tick marks to the given values

#- smoothWindowSize = 20:  sets the window size used to smooth the actual OD measurements to 20 readings.  
###########################################
...
##you'll need to got to: Packages & Data > Package Installer >> install 'lattice' and 'reshape'
library(lattice)
library(reshape)
######
...
analyzeGrowthCurves <- function(dataFileName, singleWell=NULL, oneResourceWells=c(), twoResourceWells=c("ALL"), plotDetail=F, reload=T, smoothWindowSize=7, maxOD=0.45, ODTicks=c(0.1,0.2,0.4), maxTimeHours=15, timeTicks=c(0,5,10,15), maxWells=96, selwyn=F) {
    ...
    if (reload) {
        ...
        for (wellName in wellNames) {
            ...
            meanODLastThird = mean(log(newdata$OD[newdata$Well==wellName][floor(nTimePoints*2/3):nTimePoints]))
            hasGrowth = meanODLastThird > log(minOD)

            if (!hasGrowth) {
                message(wellName, ": \tNA")
                if (is.null(singleWell)) {
                    write(c(wellName, "NA", "NA", "NA", "NA", "NA"), analysisFileName, ncolumns=6, append=T)  #####Changed '6' from '5' and added one ',"NA' so output has no empty spaces --- doesn't work???
                }
            } else {
                baseOD = mean(log(newdata$OD[newdata$Well==wellName][1:6]))
                ...
                if (isTwoResourceWell(oneResourceWells, twoResourceWells, wellName)) {
                    ...
                    message("\t\t", sprintf("%i ", expGrowth1))
                    if (is.na(expGrowth1) || length(expGrowth1) < 9) {
                        message("\t\t-->")
                        expGrowth1=(which(tmpLeft$Rate==peak1$Rate[1])-4):(which(tmpLeft$Rate==peak1$Rate[1])+4)
                        message("\t\t", sprintf("%i ", expGrowth1))
                    }
                    ...
                    message("\t\t", sprintf("%i ", expGrowth2))
                    if (is.na(expGrowth2) || length(expGrowth2) < 9) {
                        message("\t\t-->")
                        expGrowth2=((which(tmpRight$Rate==peak2$Rate[1])-4):(which(tmpRight$Rate==peak2$Rate[1])+4)) + which(tmp$Time==valley$Time[1]) - 1
                        message("\t\t", sprintf("%i ", expGrowth2))
                    }
                    ...
                    if ((abs(valleyIndex - peak1Index) <= 5) || (abs(valleyIndex - peak2Index) <= 5) || ((valley$OD[1] > peak2$OD[1]) && (valley$NRate[1] < -0.15))) {
                        ...
                        expGrowths <<- rbind(expGrowths, data.frame(Well=wellName, Left1=expGrowth1[1], Right1=expGrowth1[length(expGrowth1)], Left2=expGrowth2[1], Right2=expGrowth2[length(expGrowth2)]))
                ...
                if (!twoResourcesDetected) {
                    ...
                    rates = newdata$Rate[newdata$Well==wellName]
                    expGrowth = which(rates > max(rates)-max(rates)/6)
                    # expGrowth = which(rates>quantile(rates[rates>mean(rates)],.7))
                    
                    model = lm(log(newdata$OD[newdata$Well==wellName][expGrowth]) ~ newdata$Time[newdata$Well==wellName][expGrowth])
                    # print(summary(model))
                    c = coef(model)

                    expGrowths <<- rbind(expGrowths, data.frame(Well=wellName, Left1=expGrowth[1], Right1=expGrowth[length(expGrowth)], Left2=NA, Right2=NA))
                    models[[length(models)+1]] <<- list(Model1=model)
                    names(models) <<- c(names(models)[1:(length(models)-1)], wellName)

                    lagStartTime = newdata$Time[newdata$Well==wellName][1]
                    ...
                    if (is.null(singleWell)) {
                        write(c(wellName, sprintf("%.3f", lag), sprintf("%.9f", c[2]), "NA", "NA", "NA"), analysisFileName, ncolumns=6, append=T)
                    }
                }
            }
        }
    ...
    minRate = min(newdata$Rate)*60*60
    maxRate = max(newdata$Rate)*60*60
    ...
    plot = xyplot(OD ~ Time|Well, newdata, layout=grid, type='l', as.table=T, strip=F, xlim=c(minX,maxX), ylim=c(minY,maxY), xlab="Time (h)", main=paste(dataFile,".txt",sep=""), scales = list(x=list(at=xTicks, labels=timeTicks, tck=0.7), y=list(log="e", at=yTicks, tck=0.7)), panel = function(x, y, subscripts,...) {
        ...
        # check if growth occured in well
        meanYLastThird=mean(y[floor(length(x)*2/3):length(x)])
        hasGrowth = meanYLastThird > log(minOD)
        ...
        if (!hasGrowth) {
            ...
            panel.rect(minX, log(minY), maxX, log(0.8), col=rgb(1,1,0,0.12))
        } else {
            baseOD = baseODs$OD[baseODs$Well==wellName]
            
            # plot dashed green line for base OD level
            panel.segments(minX, baseOD, maxX*0.5, baseOD, lty=2, col=modelCol, lwd=lwdMultiplier*0.8)

            if (hasTwoGrowthPhases) {
                # plot dashed green line for OD level of beginning of 2nd lag time (valley)
                panel.segments(lags$Lag2StartTimeExt[lags$Well==wellName]-maxX*0.05, lags$Lag2OD[lags$Well==wellName], lags$Lag2EndTime[lags$Well==wellName]+maxX*0.15, lags$Lag2OD[lags$Well==wellName], lty=2, col=modelCol, lwd=lwdMultiplier*0.8)

                # plot red background for wells with problems
                if (mean(abs(residuals(models[[wellName]][["Model1"]]))) > 0.02 || mean(abs(residuals(models[[wellName]][["Model2"]]))) > 0.02 || wellName %in% wellsWithZeros || baseODs$OD[baseODs$Well==wellName] > quantile(y,.1)) {
                    panel.rect(minX, log(minY), maxX, log(0.8), col=rgb(1,0,0,0.12))
                }
                ...
                if (isTwoResourceWell(oneResourceWells, twoResourceWells, wellName)) {
                    panel.rect(minX, log(minY), maxX, log(0.8), col=rgb(0,0,1,0.06))
                }
                
                # plot red background for wells with problems
                if (mean(abs(residuals(models[[wellName]][["Model1"]]))) > 0.02 || wellName %in% wellsWithZeros || baseODs$OD[baseODs$Well==wellName] > quantile(y,.1)) {
                    panel.rect(minX, log(minY), maxX, log(0.8), col=rgb(1,0,0,0.12))
                }

                if (plotDetail) {
                    wellData = newdata[newdata$Well==wellName,]
                    panel.points(wellData$Time[wellData$Rate==max(wellData$Rate)], log(wellData$OD[wellData$Rate==max(wellData$Rate)]), col=modelCol, cex=cexPointMultiplier*0.6, lwd=lwdMultiplier*0.5)
                }
            }
        }
        ...
        if (hasGrowth) {
            ...
            if (hasTwoGrowthPhases) {
                ...
                # panel.abline(v=x[expGrowth[1]], lty=3, col=modelCol)
                # panel.abline(v=x[expGrowth[length(expGrowth)]], lty=3, col=modelCol)
    ...
    if (plotDetail) {
        ...
        plot = xyplot(SmoothedOD ~ Time|Well, newdata, layout=grid, type='l', as.table=T, strip=F, xlim=c(minX,maxX), ylim=c(minY,maxY), xlab="Time (h)", ylab="OD (smoothed)", main=paste(dataFile,".txt",sep=""), scales = list(x=list(at=xTicks, labels=timeTicks, tck=0.7), y=list(log="e", at=yTicks, tck=0.7)), panel = function(x, y, subscripts,...) {
            ...
            if (!hasGrowth) {
                # plot yellow background for wells with no growth
                panel.rect(minX, log(minY), maxX, log(0.8), col=rgb(1,1,0.0,0.12))
            } else {
                if (hasTwoGrowthPhases) {
                    # plot red background for wells with problems
                    if (mean(abs(residuals(models[[wellName]][["Model1"]]))) >= 0.02 || mean(abs(residuals(models[[wellName]][["Model2"]]))) >= 0.02 || wellName %in% wellsWithZeros || baseODs$OD[baseODs$Well==wellName] > quantile(y,.1)) {
                        panel.rect(minX, log(minY), maxX, log(0.8), col=rgb(1,0,0.0,0.12))
                    }
                    ...
                    wellData = newdata[newdata$Well==wellName,]
                    panel.points(wellData$Time[wellData$Rate==max(wellData$Rate)], log(wellData$SmoothedOD[wellData$Rate==max(wellData$Rate)]), col=modelCol, cex=cexPointMultiplier*0.6)
                }
            }
            ...
            if (hasGrowth) {
                ...
                if (hasTwoGrowthPhases) {
                    ...
                    panel.text(x=maxX+5, y=log(slopeValue1Y), pos=2, labels=paste("r:", formatC(mean(abs(residuals(models[[wellName]][["Model1"]]))), digits=3, flag="#", format = "f")), col="black", cex=cexMultiplier*0.47)
                    panel.text(x=maxX+5, y=log(slopeValue2Y), pos=2, labels=paste("r:", formatC(mean(abs(residuals(models[[wellName]][["Model2"]]))), digits=3, flag="#", format = "f")), col="black", cex=cexMultiplier*0.47)
                    ...
                    panel.text(x=maxX+5, y=log(slopeValue2Y), pos=2, labels=paste("r:", formatC(mean(abs(residuals(models[[wellName]][["Model1"]]))), digits=3, flag="#", format = "f")), col="black", cex=cexMultiplier*0.47)
                }
            }
        ...
        # plot = xyplot(Rate*60*60 ~ Time|Well, newdata, layout=grid, type='l', as.table=T, strip=F, xlim=c(minX,maxX), ylim=ratesYLim, xlab="Time (h)", ylab="Growth rate (per hour)", main=paste(dataFile,".txt",sep=""), scales = list(x=list(at=xTicks, labels=timeTicks, tck=0.7), y=list(at=ratesYTicks, tck=0.7)), panel = function(x, y, subscripts, ...) {
            ...
            if (hasTwoGrowthPhases) {
                ...
                panel.points(x[y==max(y)], max(y), col=modelCol, pch=19, cex=cexPointMultiplier*0.08)
                panel.points(x[y==max(y)], max(y), col=modelCol, cex=cexPointMultiplier*0.7)

                # panel.abline(h=max(y)-0.001, lty=3, col="green", lwd=lwdMultiplier*0.8)
                panel.abline(h=max(y)-max(y)/6, lty=3, col=modelCol, lwd=lwdMultiplier*0.8)
